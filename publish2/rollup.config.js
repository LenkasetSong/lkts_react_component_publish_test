...
// PostCSS plugins
// 处理css定义的变量
// import simplevars from 'postcss-simple-vars'; // 2020-10-23 postcss8不支持rollup
// 处理less嵌套样式写法
...
export default {
  ...
  output: [{
    file: `dist/dc-umd${env === 'production' ? '.min' : ''}.js`,
    format: 'umd',
    name: 'dcui',
  }, {
    file: `dist/dc-es${env === 'production' ? '.min' : ''}.js`,
    format: 'es'
  }],
  ...
  // 将模块视为外部模块，不会打包在库中
  external: ['antd', 'react', 'react-dom', 'prop-types'],
  // 插件
  plugins: [
    image(),
    // less(),
